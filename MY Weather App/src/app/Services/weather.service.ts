...
export class WeatherService {
  ...
  getData() {
    ...
    const loadData$ = this.getLocationDetails(this.cityName, this.language).pipe(
      tap(response => {
        this.locationDetails = response;
        latitude = this.locationDetails?.location.latitude[0];
        longitude = this.locationDetails?.location.longitude[0];
      }),
      switchMap(() => this.getWeatherReport(this.date, latitude, longitude, this.language, this.units)),
      tap(response => {
